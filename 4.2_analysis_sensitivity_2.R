...
# fit survival model on study time scale and control for survey age
...
# path_to_box <- "C:/Users/yingyan_wu/Box/"
...
# note the different specifications
surv_obj <- "Surv(MAIN_DEM_V1_FU_TIME, MAIN_DEM_V1_END_DEM_FLAG) ~ SURVEY_AGE+"
...
for (i in 1:6) {
  write.xlsx(
    ...
    file = paste0(path_to_box, 
                  ...
                  paste0("sens_study_time_scale_cox_5yr_M", i, ".xlsx")))
}

# ---- Aalen Additive Hazards Models ----

# set up model formulas ---- 
surv_obj <- "Surv(MAIN_DEM_V1_FU_TIME, MAIN_DEM_V1_END_DEM_FLAG) ~ const(SURVEY_AGE)+"
...
for (i in 1:6) {
  write.xlsx(
    ...
    file = paste0(path_to_box, 
                  "Asian_Americans_dementia/Manuscripts/AA_ADRD_diabetes/",
                  "Code/Cleaned_Scripts/output/sensitivity/",
                  paste0("sens_study_time_scale_aalen_5yr_M", i, ".xlsx")))
}
